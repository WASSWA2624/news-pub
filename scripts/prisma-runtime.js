...
function assertLocalDatabaseUrlReady(databaseUrl = process.env.DATABASE_URL) {
  ...
  if (!normalizedUrl) {
    throw new Error(
      [
        ...
        "Copy .env.example to .env.development and point DATABASE_URL at a reachable local MySQL or MariaDB database before running database-backed workflows.",
      ].join("\n"),
    );
  }
  ...
  if (!parsedUrl) {
    throw new Error(
      [
        ...
        "Update .env.development so DATABASE_URL uses a local MySQL or MariaDB connection string such as mysql://user:password@localhost:3306/news_pub.",
      ].join("\n"),
    );
  }
  ...
  if ([username, password, database].some(looksLikePlaceholderEnvValue)) {
    throw new Error(
      [
        "DATABASE_URL in .env.development still contains placeholder credentials.",
        `Current target: ${describeDatabaseTarget(normalizedUrl)}.`,
